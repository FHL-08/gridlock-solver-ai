...
serve(async (req) => {
  ...
  // Rate limiting: 20 requests per minute per IP (chat can be more frequent)
  ...
  try {
    ...
    console.log(`[Ambulance Chat]: Message from ${clientId} - length: ${message.length}`)
...
Patient context: ${patientContext}`
    ...
    console.error(`[Ambulance Chat]: Error from ${clientId}:`, error)
